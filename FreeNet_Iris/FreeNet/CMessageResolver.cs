...
namespace FreeNet
{
    ...
    class CMessageResolver
    {
        ...
        public void on_receive(byte[] buffer, int offset, int transffered, CompletedMessageCallback callback)
        {
            ...
            Console.WriteLine($"[IRIS] on_receive transffered: {transffered}");
            // 원본 버퍼의 포지션값.
            ...
            while (this.remain_bytes > 0)
            {
                ...
                if (this.current_position < Defines.HEADERSIZE)
                {
                    ...
                    Console.WriteLine($"[IRIS] 1 buffer startIndex:{current_position}, remain_bytes: {remain_bytes}");
                    ...
                    Console.WriteLine($"[IRIS] 2 buffer startIndex:{current_position}, remain_bytes: {remain_bytes}");
                    // 헤더 하나를 온전히 읽어왔으므로 메시지 사이즈를 구한다.
                    this.message_size = get_total_message_size();
                    Console.WriteLine($"[IRIS] message_size: {message_size}");
                ...
                for (int i = 4; i <= 18; i++)
                {
                    Console.WriteLine($"message[{i}] = {message_buffer[i]}");
                }
